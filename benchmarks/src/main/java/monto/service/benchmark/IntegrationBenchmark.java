...
public class IntegrationBenchmark extends Benchmark {
    ...
    while (true) {
      ProductMessage prod =
          sink.<ProductMessage, RuntimeException>receive(
              p -> p,
              disc -> {
                throw new RuntimeException("Unexpected discovery response");
              });
      if (prod.getServiceId().equals(serviceId)) return prod.getTime();
    }
  }
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (ServiceId service :
        Arrays.asList(
            JavaServices.HIGHLIGHTER, JavaServices.JAVACC_PARSER, JavaServices.OUTLINER)) {
      Path csvOutput = csvOutputDir.resolve(service.toString() + ".csv");
      IntegrationBenchmark bench =
          new IntegrationBenchmark(
              brokerPath, servicesJar, service, "-highlighter", "-javaccparser", "-outline");
