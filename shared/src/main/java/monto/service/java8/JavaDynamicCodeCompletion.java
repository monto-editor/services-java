...
public class JavaDynamicCodeCompletion extends MontoService {
  ...
  private Set<DynamicDependency> findDynamicDependencies(
      SourceMessage source, Set<Source> requiredSources) throws ParseException {
  ...
  private List<Tuple> getDynamicDependencyMessages(Request request, Set<Source> requiredSources)
      throws ParseException {
    List<Tuple> msgs = new ArrayList<>();
    for (Source s : requiredSources) {
      ProductMessage astMessage =
          request
              .getProductMessage(s, Products.AST, Languages.JAVA)
              .orElseThrow(
                  () -> new IllegalArgumentException("No AST message in request for " + s));
      msgs.add(
          new Tuple(
                  ...
                  .orElseThrow(
                      ()
                          -> new IllegalArgumentException(
                              "No Source message in request for " + s))));
    }
    return msgs;
  }
  ...
  private void sendMissingRequirements(SourceMessage source, Set<DynamicDependency> dynDeps) {
    RegisterDynamicDependencies regDynDep =
        new RegisterDynamicDependencies(source.getSource(), JavaServices.CODE_COMPLETION, dynDeps);
  ...
  private class AllCompletions implements ASTVisitor {
    ...
    public void visit(NonTerminal node) {
      switch (node.getName()) {
        case "packageDeclaration":
          ...
          completions.add(
              new Completion(
                  "package", packageIdentifier.extract(content), getResource("package.png")));
          break;
        ...
        case "variableDeclaratorId":
          if (fieldDeclaration) leaf(node, "field", getResource("private.png"));
          break;
    ...
    private void leaf(NonTerminal node, String name, URL url) {
      AST ident =
          node.getChildren().stream().filter(ast -> ast instanceof Terminal).findFirst().get();
